...
const CategoryList = ({ currentCategory, categories }) => {
  ...
  return <ul className='flex py-2 pb-12 space-x-3 overflow-x-scroll'>
    <Link href={`../`} passHref>
    <li className='cursor-pointer duration-200 mr-1 my-1 pl-5 px-1 py-1 font-medium text-3xl whitespace-nowrap text-gray-300 hover:text-gray-700 dark:hover:text-gray-300 dark:text-gray-600 '>전체</li>
    </Link>
        ...
        <Link key={category.name} href={`/category/${category.name}`} passHref>
          <li
            className={`cursor-pointer duration-200 mr-1 my-1 px-1.5 py-1 font-medium text-3xl whitespace-nowrap
                ...
                : 'text-gray-300 hover:text-gray-700 dark:hover:text-gray-300  dark:text-gray-600'
              }`}
          >
            <a>
              {`${category.name}`}
